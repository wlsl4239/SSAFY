...
def preprocess_data(filename):
    ...
    with open(filename) as file:
        ...
        for title,user in loaded.items():
...
def predict_preference(title_to_users, user_to_titles, user, title):
    ...
    closeness = [get_closeness(title_to_users,i,title) for i in titles]
...
'''
넷플릭스 시청 데이터 분석하기
3주차 프로젝트에서는 넷플릭스의 작품 별 시청 데이터를 분석해 봅니다.
